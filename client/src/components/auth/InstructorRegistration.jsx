...
export default function InstructorRegistration() {
  ...
  const handleSubmit = async(event) => {
    ...
    try {
      const response = await axios.post('http://localhost:7000/auth/register', instructorAuth)
      if (response.data.success) { 
        try {
          ...
          const instructorResponse = await axios.post('http://localhost:8000/instructor/create', instructorDetails);
          alert('Succesfully registerd as instructor')
          window.location.pathname = '/instructor-sign-in'
        } catch (err) { 
